...
function modalView(s: AppState): string {
  ...
  return `<dialog id="app-dialog" class="${modal.kind === "image" ? "wide" : ""}" aria-labelledby="dialog-title"><div class="dialog-heading"><h2 id="dialog-title">${e(title)}</h2><button class="close-button" data-action="close-modal" aria-label="閉じる"${disabled(s)}>×</button></div>${s.error ? `<div class="error" role="alert">${e(s.error)}</div>` : ""}${body}</dialog>`;
}
